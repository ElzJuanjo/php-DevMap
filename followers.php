...
<body>
    ...
    <main>
        <div class="formulario">
            <?php
            echo "<h1 class='titulo'>Seguidores de $correo</h1>";
            $sql = "SELECT seguidor FROM seguidor WHERE seguido = '$correo'";
            $consulta = $conexion->query($sql);
            $resultado = $consulta->num_rows;
            if ($resultado > 0) {
                while ($usuario = $consulta->fetch_assoc()) {
                    $user = $usuario['seguidor'];
                ...
                echo "<br>Este usuario no tiene seguidores.";
            }
            ?>
        </div>
    </main>
